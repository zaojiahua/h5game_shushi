...
(function () {
    ...
    // extend是cc.Class的静态方法
    cc.Class.extend = function (props) {
        // this代表的是cc.Class或者继承自cc.Class的类
        var _super = this.prototype;
        // Object.create创建对象的一种新方式，将_super作为父类 返回
        // prototype，这个prototype
        // 将作为创建的新类的原型
        var prototype = Object.create(_super);

        // 记录一个新的类，classid代表的是新类的id，对应的是它的父类
        ...
        // 定义prototype的一个属性__pid 值为classId
        desc.value = classId;
	    Object.defineProperty(prototype, '__pid', desc);
	    
        // 这里的class就是最后创建出来的新类（定义的类），最后将这个创建出来的类返回
        function Class() {
            this.__instanceId = ClassManager.getNewInstanceId();
            // new出来对象的时候调用构造函数，传入new时候的所有参数
            if (this.ctor)
                this.ctor.apply(this, arguments);
        }
        Class.id = classId;
        // 将prototype作为要创建的新类的原型
        Class.prototype = prototype;
	    
        // 定义prototype的constructor属性是新创建的类
        desc.value = Class;
        ...
        // 对传入的参数进行处理 传入的参数就是自定义类的一些属性
        // arguments是类数组对象，有length属性
        // 这个for循环对传入的参数进行遍历
        for(var idx = 0, li = arguments.length; idx < li; ++idx) {
            // 获得属性，我们的第一个参数一般传递一个对象，里边包含类的所有属性
            ...
            for (var name in prop) {
                ...
                // 如果覆写了父类的函数，并且在该函数中调用了_super方法
                // 重新包装该函数
                if (isFunc && override && hasSuperCall) {
                    desc.value = (function (name, fn) {
                        // 返回一个包装了的函数，在这个函数内部
                        // 改变了this._super的值，然后调用了fn函数，传入了
                        // 改变了_super属性的this
                        return function () {
                            var tmp = this._super;
                ...
                // 如果是函数，无论是否进行了重写，都添加到prototype对象中
                } else if (isFunc) {
                    desc.value = prop[name];
                    Object.defineProperty(prototype, name, desc);
                // 普通属性 也添加到prototype中 属性没有添加到class中
                // 而是添加到了prototype中，原理是当new出来一个对象的时候
                // 我们可以在ctor或者其他函数中，重写这个对象自己的属性值
                // 如果不重写，那么属性值默认用的就是父类的属性值，这样做可以避免
                // 内存空间的浪费。
                } else {
